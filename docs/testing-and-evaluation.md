...
| --- | --- | --- |
| Domain unit | Workflow/goal/budget/concurrency/config compilation, exact condition/guard/join validation, concurrent capacity and dependency replay, declaration-ordered outcomes and failure projection, durable decision and omission replay, fresh-recovery policy bounds, monotonic attempt replay, proof-safe interruption eligibility, monotonic capacity merge, admission state-machine exploration, checked resource aggregation, exhaustion, policy and approval operation digests, pure criterion evaluation, and decision/effect/settlement/reconciliation/receipt/approval replay invariants | None |
| Application unit | Quiescent-wave admission, overlap bounds, reverse completion with declaration order, selected-branch concurrency, approval/control barriers, cancellation and failure quiescence, settlement-ceiling precedence, scheduler ordering, omission propagation, serialized write-ahead effect publication, ordered multi-attempt recovery observation/disposition, crash boundaries around the resume marker, partial reconciliation progress, recovery compatibility, budget stop boundaries and timeout clamping, approval waits and expiry, completed-node skipping, failure propagation, and executor authority | Test-only in-memory ports |
