...
| --- | --- | --- |
| Domain unit | Workflow/goal/budget/config compilation, exact condition/guard/join validation, durable decision and omission replay, fresh-recovery policy bounds, monotonic attempt replay, proof-safe interruption eligibility, monotonic capacity merge, admission state-machine exploration, checked resource aggregation, exhaustion, policy and approval operation digests, pure criterion evaluation, and decision/effect/settlement/reconciliation/receipt/approval replay invariants | None |
| Application unit | Scheduler ordering, selected-branch execution, omission propagation, resource-neutral control transitions, conditional crash recovery, serialized write-ahead effect publication, ordered and idempotent recovery observation/disposition, crash boundaries around the resume marker, partial reconciliation progress, recovery compatibility, budget stop boundaries and timeout clamping, approval waits and expiry, completed-node skipping, failure propagation, and executor authority | Test-only in-memory ports |
