...
| --- | --- | --- |
| Domain unit | Workflow/goal/budget/concurrency/config compilation, exact condition/guard/join/approval and bounded-loop expansion validation, persisted loop topology, concurrent capacity and dependency replay, declaration-ordered outcomes and failure projection, durable decision/check/completion/omission replay, fresh-recovery policy bounds, monotonic attempt replay, proof-safe interruption eligibility, monotonic capacity merge, admission state-machine exploration, checked resource aggregation, exhaustion, policy and command/graph approval digests, pure criterion evaluation, and decision/effect/settlement/reconciliation/receipt/approval replay invariants | None |
