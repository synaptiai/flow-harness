...
Gates 0–2 are implemented. Gate 3 has a model-tool policy broker, a full-SHA hash-anchored single-file edit, durable effect receipts, write-ahead argv-only agent command execution through fail-closed native containment, exact expiring approval for deterministic command nodes, and exact per-call approval for live agent commands. Gate 4 has committed-boundary recovery, exclusive same-host ownership, write-ahead durable edit evidence, typed reconciliation of open hash-anchored edits, opt-in proof-safe fresh recovery of interrupted agent attempts, client-detachable approval waits, durable limits for node starts, model tokens, reported model cost, active execution time, and retained executor-output artifacts, project/operator capacity configuration, plus a bounded local supervisor with authenticated detached workers, durable FIFO admission, queued and active cancellation, bounded event replay, policy-safe restart, and worker adoption. Gate 5 has replay-safe exact-output conditions, guarded branches, first-class omission, explicit joins, deterministic bounded concurrency for static DAG forks, finite replay-safe bounded and accept-best optimization loops, durable evidence-bound graph approval nodes, replay-verified typed results, typed verifier nodes with command and zero-tool model drivers, independently-ledgered child workflows, and write-ahead candidate promotion in isolated copy-on-write-or-copy workspaces. Gate 6 has strict local and digest-pinned installed Agent Skills, versioned command/model verifier packages, declarative command tool packages, and inert versioned workflow source packages. Deterministic inert bundles, explicit public-HTTPS-plus-SHA-256 installation, a content-addressed project store, collision-checked catalogs, local audit/removal, and offline run/recovery paths are implemented. Reproducible harness evaluation is implemented for paired Flow workflow profiles with frozen controls, fresh fixture copies, private deterministic verification, digest-chained evidence, offline reports, and constrained paired comparison. Evidence-bound prompt candidates are implemented with canonical tuning-only evidence, exact supplemental prompt overlays, ordinary compiler projection, and candidate-aware paired evaluation; automatic generation and activation remain open. Executable extensions, signed registries and automatic updates, policy/UI package types, arbitrary evaluator runtimes, model network tools, broader configurable policy, opaque session continuation, general failure/fallback retries, external artifact storage/retention, additional native evaluation adapters, and stronger VM or managed backends remain target capabilities.
...
- Prompt candidates are evaluated against held-out and regression workflows through the existing paired evaluation gate. *(Implemented; evaluation is evidence and does not activate the candidate.)*
