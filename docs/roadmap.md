...
- Explicit agent `exec` uses the same sandboxed command executor, exact process authorization, write-ahead prepare/settle events, bounded output evidence, and replay-derived artifact accounting. *(Implemented; dynamic per-call human approval remains separate.)*
