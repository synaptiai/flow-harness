...
| Restricted relay source admission, development only | `native/verification-relay/` and `test/runtime/verification-relay-source.runtime.test.ts` | Captures exact source bytes against a reviewed Flow-owned manifest through the separate `--check-relay-sources` mode. Rejects altered inputs and invalid inventories. It does not download, build, execute, or admit a runtime artifact. The [runtime custody design](host-bridge-runtime-custody-design.md) owns the restricted profile and remaining gates. |
