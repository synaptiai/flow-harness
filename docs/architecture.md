...
flowchart TB
    ...
    corpus -->|"Ships with the archive"| channels
...
0. The reviewed package manifest owns one release version. Automation requires the shrinkwrap and
   ...
   provenance and protected publication make it available.
...
| --- | --- | --- |
| Preview release automation | `package.json`, `npm-shrinkwrap.json`, `scripts/resolve-preview-release-identity.mjs`, `src/domain/release/`, `src/infrastructure/release/`, `scripts/build-package-release.mjs`, `scripts/verify-package.mjs`, and `.github/workflows/preview-release.yml` | Validates one manifest-owned identity, derives every release name, builds one bounded archive, records its installed-file identity, verifies the same archive on supported x64 hosts, generates provenance, and gates immutable publication. |
...
| Compatibility boundary | `src/domain/compatibility/check.ts`, `src/infrastructure/compatibility/local-corpus.ts`, `src/cli/main.ts`, `compatibility/`, `src/domain/release/package-release-evidence.ts`, `src/infrastructure/release/package-release-verifier.ts`, and `scripts/verify-package.mjs` | Keeps npm imports closed, reads one bounded no-follow package corpus, reuses the production compiler and run reducer, emits content-free per-artifact results, and verifies the behavior from the packed archive. |
