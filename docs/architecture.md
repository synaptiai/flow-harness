...
flowchart TB
    ...
    subgraph delivery["0. Distribution — publishes one reviewed package"]
        direction LR
        releaseIdentity["Reviewed release identity<br/>Manifest · matched shrinkwrap · notes"]
        release["GitHub preview automation<br/>Build once · test x64 · attest"]
        reference["Generated capability reference<br/>Stable JSON · readable Markdown"]
        corpus["Immutable compatibility corpus<br/>Historical workflow · terminal run"]
        githubRelease["Immutable GitHub prerelease<br/>Exact archive · evidence · provenance"]
        npmStage["npm staging automation<br/>Reverify bytes · short-lived authority"]
        npmApproval["Human npm approval<br/>Compare archives · confirm with 2FA"]
        channels["Public npm preview channel<br/>Exact approved archive"]
    end
    ...
    subgraph access["1. Ways to use Flow"]
        ...
        compatibility["Compatibility check<br/>Offline · read-only · content-free report"]
    ...
    subgraph control["2. Control plane — decides what can happen"]
        ...
        issueRepair["Optional bounded review repair<br/>Retain reviewed parent · reserve resources · stop on uncertainty"]
    ...
    subgraph observerChecks["Verification-observer qualification — not connected to issue runs"]
        observerCI["Focused hosted Linux development job<br/>Separate from full CI and release gates"]
        ...
        observerLaunch["Observer launch rewrite<br/>Exact helper and proxy templates<br/>Does not start a process"]
        observerNative["Native observer extension and reproducible build<br/>Exact application execution and private reports<br/>Not yet qualified"]
        observerMutant["Separate deliberately broken test executable<br/>Tests whether false success is detected<br/>Never used for issue execution"]
        bridgeOwner["Native host-bridge owner<br/>Private release protocol · owned child reaping<br/>Development only"]
        bridgeRelay["Trusted host relay under test<br/>Real local socket forwarding"]
        relaySourceCheck["Restricted relay source admission<br/>Reviewed manifest · captured source bytes<br/>No build or runtime qualification"]
        relayBaseline["Static relay comparison baseline<br/>Two clean builds · source and link evidence<br/>Unwrapped comparison executable"]
        relayProfile["Restricted host-relay profile<br/>Fixed arguments · environment · IPv4 resolver<br/>Unpublished · native qualification pending"]
    ...
    github -->|"Runs an explicitly scoped feedback branch"| observerCI
    observerCI -->|"Executes the native result-path regression"| observerTests
    ...
    observerTests -->|"Rejects malformed or mismatched records"| observerResult
    observerTests -->|"Checks exact launch arguments"| observerLaunch
    observerLaunch -->|"Reuses the admitted Linux launch parser"| commands
    observerTests -->|"Checks real execution, frame bytes, and build inputs"| observerNative
    observerNative -->|"Builds a separately identified executable"| bridgeOwner
    observerTests -->|"Checks forwarding and release receipts"| bridgeOwner
    observerTests -->|"Checks real release inputs and altered-file rejection"| relaySourceCheck
    relaySourceCheck -->|"Supplies captured inputs for compilation"| relayBaseline
    observerTests -->|"Tests restricted-option rejection against the real baseline"| relayBaseline
    relayBaseline -->|"Supplies an explicit test-only relay selection"| bridgeOwner
    relaySourceCheck -->|"Captures the separately selected wrapper recipe"| relayProfile
    observerTests -->|"Checks rejection and real profile forwarding"| relayProfile
    relayProfile -->|"Supplies a separate test-only relay selection"| bridgeOwner
    observerTests -->|"Checks startup failures and real leftover controls"| bridgeStartup
    observerTests -->|"Checks owner loss without crediting later namespace cleanup"| bridgeStartup
    ...
    observerProcessOracle -->|"Supplies real observations for decision calibration"| bridgePredicate
    ...
    people -->|"Approves immutable publication"| release
    release -->|"Publishes one verified archive"| githubRelease
    githubRelease -->|"Supplies the exact release assets"| npmStage
    npmStage -->|"Creates a private staged version"| npmApproval
    people -->|"Reviews the staged bytes"| npmApproval
    npmApproval -->|"Publishes after proof of presence"| channels
    release -->|"Checks generated bytes"| reference
    release -->|"Checks historical semantics"| corpus
    descriptors -->|"Generates stable JSON and Markdown"| reference
    descriptors -->|"Defines public runtime surfaces"| agents
    reference -->|"Ships with the archive"| githubRelease
    corpus -->|"Ships with the archive"| githubRelease
    githubRelease -->|"Installs verified release bytes"| cli
    githubRelease -->|"Installs packaged check and corpus"| compatibility
    channels -->|"Installs exact package bytes"| cli
    channels -->|"Installs packaged check and corpus"| compatibility
    people -->|"Starts attached work"| cli
    people -->|"Starts the first bounded run"| quickstart
    people -->|"Checks a selected path"| diagnostics
    people -->|"Checks historical artifacts"| compatibility
    ...
    compatibility -->|"Replays only packaged bytes"| corpus
    ...
    issueLifecycle -->|"Selects only preauthorized blocked reviews"| issueRepair
    issueRepair -->|"Runs one fixed repair allowance"| engine
    issueRepair -->|"Reads exact committed finding locations"| issueHost
    issueRepair -->|"Records child usage and candidate trees before commit"| issueRuns
...
0. The reviewed package manifest owns one release version. Automation requires the shrinkwrap and
   versioned notes to match, then derives every tag, archive, attestation, title, and path. It builds
   one npm archive. Shared production descriptors generate the public capability reference. The
   immutable corpus binds selected historical artifacts to their producer release. The release gate
   rejects stale reference or corpus bytes. Linux x64 and macOS x64 consume the same archive before
   provenance and protected publication create an immutable GitHub prerelease. A separate workflow
   reverifies those assets and creates a private npm stage. A maintainer compares the archives and
   uses two-factor authentication before npm makes the approved bytes public.

1. People and automation use the command line, guided quick start, read-only diagnostic,
   compatibility check, or a first-party presentation view. The compatibility check reads only
   package-owned historical artifacts. Quick start publishes one project and starts one attached
   run. Coding mode adds one reviewed fixture and an explicit tool and budget boundary.
...
| Startup cleanup witness, qualification only | `test/runtime/host-bridge-startup.runtime.test.ts` and `test/fixtures/host-bridge-startup-witness.c` | Starts the actual guardian inside a test-only PID namespace. The witness records remaining children immediately after waiting for the exact guardian and before namespace teardown. Real adopted live and unreaped children calibrate this check. A forwarding case checks the same test environment. The selected startup and owner-loss cases passed on hosted Linux x64. Owner loss records a surviving child and rejects guardian settlement without crediting later test cleanup. This does not establish unnamespaced host custody or change the ordinary manager. |
| Resistant-child observation, qualification only | `test/fixtures/host-bridge-resistant-relay.c` and `test/fixtures/host-bridge-resistance-witness.h` | Extends the existing witness without another child process. A socket supplies the connected child's kernel-bound process handle. Separately compiled resistant and cooperative fixtures distinguish actual TERM survival from successful cleanup alone. Both selected cases passed on hosted Linux x64. This tests lifecycle behavior, not traffic forwarding or production custody. |
...
| Verification-observer components, qualification only | `src/infrastructure/verification/immutable-input-fixture.ts`, `src/infrastructure/verification/linux-observer-command.ts`, `src/infrastructure/verification/native-observer-result.ts`, and `src/infrastructure/verification/native-observer-launch.ts` | Separates private fixture ownership, bounded native Linux command observation, private-frame recognition, and exact launch rewriting. The rewrite reuses the existing SRT parser and preserves admitted restrictions. Proxy-template matching requires independently admitted relay and socket identities. It does not execute a process or transfer descriptors. The decoder does not authenticate its writer or prove launch and settlement. These internal components are not connected to issue runs, do not classify behavioral failures, and cannot select repair. The [verification repair design](bounded-verification-repair-design.md) owns the remaining observer and lifecycle gates. |
| Native observer extension and reproducible build, qualification only | `native/verification-observer/` | Preserves pinned upstream source and license inputs. An explicit observer patch connects exact executable-descriptor launch, raw wait results, and three private reporting channels. The separate build mode records patch, header, source, object, and binary identities. An additional test-only mode compiles a deliberately broken source copy into separately identified artifacts under `test-controls/false-normal/`. The runtime gate must reject its false-success result without replacing the genuine observer. Native custody, execution-failure handling, and settlement remain under qualification. Ordinary SRT execution remains unchanged. |
| Native host-bridge owner, development only | `native/verification-observer/host-bridge-guardian.c` and `test/runtime/host-bridge-guardian.runtime.test.ts` | Adds a separate trusted-bridge ownership process and private release protocol. The reproducible build retains its source, object, and binary separately. Real socket tests check forwarding and selected release outcomes. `test/runtime/host-bridge-descendants.runtime.test.ts` and `test/runtime/helpers/host-bridge-probe.ts` check held-connection settlement through the independent C process oracle. The first real case passed on hosted Linux x64. Admission controls, descendant custody, full failure qualification, and observer-only SRT integration remain incomplete. The ordinary manager does not load this executable. |
| Relay source admission and static baseline, development only | `native/verification-relay/` and `test/runtime/verification-relay-source.runtime.test.ts` | Captures exact source bytes against a reviewed Flow-owned manifest. Separate modes prepare a build context and compare two native Linux x64 builds. The hosted job downloads only reviewed source URLs and preserves a separate unwrapped baseline. Selected baseline build and forwarding checks passed on Linux x64. The baseline does not implement the restricted profile or admit an executable for production. The [runtime custody design](host-bridge-runtime-custody-design.md) owns the remaining gates. |
| Restricted host-relay profile, unpublished | `native/verification-relay/restricted-relay.c`, `native/verification-relay/build.mjs`, and `test/runtime/restricted-relay.runtime.test.ts` | Admits the fixed host arguments and exact environment before upstream socat main. A separately linked resolver wrapper admits the expected call and uses numeric IPv4 loopback. The explicit profile build retains both baseline and restricted executables plus source, object, link, ELF, and license evidence. The approved wrapper-only license is Apache-2.0 OR MIT. The combined GPLv2 relay selects the MIT option without changing Flow's main license or upstream licenses. Rejection and real forwarding tests are prepared. Native compilation, runtime qualification, resolver isolation, and executable custody remain open. The ordinary SRT manager does not select this executable. |
| Native observer development check | `.github/workflows/native-observer-qualification.yml`, `test/runtime/helpers/`, and `test/fixtures/native-observer-*.c` | Runs application-result and adversarial transport checks on hosted Linux x64 from a separate feedback branch. Isolated system Python closes ambient descriptors in the test child, not in the parent runner. Tests inject an owned descriptor and apply test-only syscall restrictions before the real helper. Separate host-only helpers observe fixed descendants and calibrate live, zombie, and reaped states without signaling discovered processes. Cancellation controls use a trusted launcher that signals only its own helper child and closes parent copies of private descriptors. The independent process checker remains active across observation cancellation. Writer-access controls target the actual private report descriptor. These tests do not change production policy. This workflow does not replace full CI, publish artifacts, invoke models, or qualify repair enablement. The [testing guide](testing-and-evaluation.md#develop-the-native-result-transport-independently) owns its scope and retained-evidence rules. |
...
- Change the npm export boundary, compatibility corpus, or cross-release check.
...
| Check compatibility | An operator runs `flow compatibility check` from any directory | One bounded report for every package-owned historical artifact; no project discovery, provider call, network access, or mutation |
...
GitHub release creation and npm publication are separate recovery domains. The preview-release
workflow builds one archive and verifies the same bytes on both supported release hosts. It then
generates provenance and publishes a complete immutable GitHub prerelease through a protected
environment. It doesn't publish npm.

The npm-stage workflow starts from the immutable tag and assets rather than rebuilding source. Its
unprotected job validates the release record, exact source revision, asset set, and provenance
before it executes release-source build code. It then validates installed behavior, the import
boundary, and the unused npm version. Only after every check can a second protected job receive
short-lived stage-only npm authority. It downloads and verifies the archive again, creates a staged
version, and proves that public distribution tags didn't move. A human remains responsible for
comparing the staged and GitHub bytes and for approving or rejecting the stage with two-factor
authentication.

This separation lets npm staging recover after a registry outage without recreating or weakening an
immutable GitHub release. It also prevents a CI job from turning a verified prerelease into a public
npm version without a distinct proof-of-presence decision.

### Compatibility boundary

The npm package exposes one executable and an empty `exports` map. Package-root and undeclared
subpath imports fail before they can load Flow internals. The executable needs the compiled files,
so they remain in the archive. Their physical presence doesn't make them a library API.

The compatibility corpus is package-owned, versioned, and immutable. Its manifest binds each
artifact path, digest, kind, producer package, producer version, producer archive digest, and exact
expected observations. The local reader rejects linked corpus and intermediate directories. It opens
the manifest and artifacts without following final links or waiting on special files. It applies
fixed byte, depth, node, path, and artifact-count limits and compares file identity before and after
each read.

The domain checker uses the ordinary workflow compiler and run-event reducer. It verifies source
digests before parsing and compares content-free semantic observations. It retains one ordered
result per declared artifact and fails the overall result if any artifact isn't established. It
doesn't rewrite the historical source or discover a project. It also doesn't take run ownership,
invoke a provider, or contact a network service.

Release evidence admits the `compatibility/` subtree and requires its manifest. The clean archive
gate installs the exact package, runs the check, and proves that package-root and internal subpath
imports fail. Read the [Compatibility policy](compatibility.md) and
[Library API assessment](library-api-assessment.md).
...
| Compatibility manifest is missing, malformed, oversized, unsupported, or changes during read | Return one stable value-free diagnostic, modify no project or corpus bytes, and make no compatibility claim |
| Compatibility artifact is missing, malformed, oversized, identity-mismatched, or semantically changed | Retain one bounded incompatible result for the declared artifact and fail the overall report |
| Package-root or undeclared-subpath import succeeds | Fail the package gate; don't publish internal modules as an accidental API |
...
- Flow does not expose its compiled JavaScript modules, application ports, stores, supervisor, or
  infrastructure adapters as a supported library API.
