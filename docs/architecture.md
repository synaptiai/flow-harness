...
| Native observer development check | `.github/workflows/native-observer-qualification.yml` | Runs application-result and adversarial transport checks on hosted Linux x64 from a separate feedback branch. Isolated system Python closes ambient descriptors in the test child, not in the parent runner. Tests inject an owned descriptor and apply test-only syscall restrictions before the real helper. They check normal exits, signals, and execution failures without changing production policy. This workflow does not replace full CI, publish artifacts, invoke models, or qualify repair enablement. The [testing guide](testing-and-evaluation.md#develop-the-native-result-transport-independently) owns its scope and retained-evidence rules. |
