...
0. The reviewed package manifest owns one release version. Automation requires the shrinkwrap and
   ...
   one npm archive. Shared production descriptors generate the public capability reference, and the
   release gate rejects stale bytes. Linux x64 and macOS x64 consume the same archive before
