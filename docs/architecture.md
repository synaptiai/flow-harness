...
flowchart TB
    ...
    subgraph observerChecks["Verification-observer qualification — not connected to issue runs"]
        ...
        observerNative["Pinned native source, build foundation, and frame encoder<br/>Observer extension not yet qualified"]
    ...
    observerTests -->|"Checks native frame bytes and build inputs"| observerNative
    observerNative -.->|"Planned protected native transport"| observerResult
...
| Observer native build foundation and frame encoder, qualification only | `native/verification-observer/` | Preserves pinned upstream source and license inputs. The separate C encoder implements the private frame format, without result-channel ownership or production integration. Source provenance and compatible frame bytes do not qualify a modified supervisor. Ordinary SRT execution remains unchanged. |
