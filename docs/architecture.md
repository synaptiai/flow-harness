...
flowchart TB
    ...
    subgraph observerChecks["Verification-observer qualification — not connected to issue runs"]
        ...
        observerNative["Pinned native source and build foundation<br/>Observer extension not yet qualified"]
...
| Observer native build foundation, qualification only | `native/verification-observer/` | Preserves pinned upstream source and license inputs for the approved observer-specific extension. Source provenance and frame parsing do not qualify a modified supervisor. Ordinary SRT execution remains unchanged. |
