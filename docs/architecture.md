...
flowchart TB
    ...
    subgraph observerChecks["Verification-observer qualification — not connected to issue runs"]
        ...
        relayBaseline["Static relay comparison baseline<br/>Two clean builds · source and link evidence<br/>Restricted profile not implemented"]
...
| Relay source admission and static baseline, development only | `native/verification-relay/`, `test/runtime/verification-relay-source.runtime.test.ts`, and `test/runtime/restricted-relay.runtime.test.ts` | Captures exact source bytes against a reviewed Flow-owned manifest. Separate modes prepare a build context and compare two native Linux x64 static baseline builds. The hosted job downloads only reviewed source URLs and tests baseline forwarding with the existing guardian. Selected baseline build and forwarding checks passed on Linux x64. A separate failing-test experiment checks the missing restricted argument boundary. This does not implement the restricted resolver profile or admit an executable for production. The [runtime custody design](host-bridge-runtime-custody-design.md) owns the remaining gates. |
