...
| Startup cleanup witness, qualification only | `test/runtime/host-bridge-startup.runtime.test.ts` and `test/fixtures/host-bridge-startup-witness.c` | Starts the actual guardian inside a test-only PID namespace. The witness records remaining children immediately after waiting for the exact guardian and before namespace teardown. Real adopted live and unreaped children calibrate this check. A forwarding case checks the same test environment. Qualification is pending. This does not establish unnamespaced host custody or change the ordinary manager. |
