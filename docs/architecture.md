...
| Native observer development check | `.github/workflows/native-observer-qualification.yml` | Runs the focused application-result regression on hosted Linux x64 from a separate feedback branch. Isolated system Python closes ambient descriptors in the test child, not in the parent runner. The test separately injects an owned descriptor to check application-entry closure. It does not replace full CI, publish artifacts, invoke models, or qualify repair enablement. The [testing guide](testing-and-evaluation.md#develop-the-native-result-transport-independently) owns its scope and retained-evidence rules. |
