...
| --- | --- | --- |
| Production TypeScript files | 382 | A broad root export would expose most of the product, not a small SDK. |
| Exported top-level declarations | 3,460 | Export syntax currently marks internal seams, test seams, schemas, records, and adapters. |
| Domain declarations | 1,633 | Even the provider-neutral layer contains large workflow, event, evaluation, package, and adaptation contracts. |
| Application declarations | 577 | Use cases expose ports for stores, executors, approvals, artifacts, workspaces, and sessions. |
| Infrastructure declarations | 1,103 | These declarations can reach files, processes, networks, sandboxes, containers, credentials, and UI hosts. |
| Supervisor declarations | 122 | These declarations own queues, worker processes, control requests, and shutdown. |
| CLI declarations | 25 | The CLI composes 346 of 382 production modules and is the intentional product boundary. |
...
| CLI composition root | 346 | All five layers | Correct executable boundary; unsuitable as an in-process API. |
