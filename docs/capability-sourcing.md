...
| Basic coding tools | Initially reuse only Pi's read-only tools; wrap later | The initial adapter does not intercept each call; future broker-facing schemas and policy checks will be Flow-owned |
