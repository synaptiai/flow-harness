...
| --- | --- | --- | --- | --- |
| 1, 2 | Candidate and admission contract | `npx vitest run test/unit/adaptation/delegation-evaluation-candidate.test.ts test/unit/infrastructure/fs/local-delegation-evaluation-candidate.test.ts test/unit/infrastructure/fs/local-delegation-evaluation-plan.test.ts test/unit/infrastructure/pi/embedded-pi-executor-registry.test.ts` | Boundary values, exact identity, no-follow reads, drift rejection, executor revalidation, and exact baseline/candidate pairing pass | General executable plugin admission |
| 3–6 | Tool, runtime, cancellation, and recovery behavior | `npx vitest run test/unit/infrastructure/pi/workspace-delegation-tools.test.ts test/unit/infrastructure/pi/pi-agent-executor.test.ts test/unit/application/run-workflow-delegation.test.ts` | Zero or one call, fixed objective, typed result, isolation, one resource charge, second-call denial, recursion denial, write-ahead intent, cancellation, reconciliation, replay rejection, cleanup, and blocked automatic retry pass | Parallel, background, or multi-host delegation |
| 7–9 | Evaluation and public evidence | `npx vitest run test/unit/evaluation/plan.test.ts test/unit/evaluation/records.test.ts test/unit/application/evaluation-adapter.test.ts test/unit/application/run-evaluation.test.ts test/unit/evaluation/aggregate.test.ts test/unit/cli/public-output.test.ts` | Task-class coverage, pre-trial identity revalidation, decomposed observations, contradiction rejection, existing verdicts, content-free inspect and export, and no activation pass | Benchmark superiority before measured held-out evidence |
