...
- Child execution already derives a deterministic run identity, creates an isolated workspace,
  records an independent ledger, rolls verified resource totals into the parent, and reopens exact
  workspace and ledger identity during recovery.
...
- Paired evaluation already binds two exact effective states and holds tasks, seeds, budgets,
...
3. Flow proves that the model route, tasks, fixtures, seeds, budgets, network policy, retries,
   immutable package bytes, and verification controls are equal.
...
6. Inspection and export use durable evidence without reopening the live candidate or catalog.
...
- The [OpenAI Agents SDK tools guide](https://openai.github.io/openai-agents-js/guides/tools/)
  describes an agent as a tool as a bounded nested run that returns its result to the owning agent.
  Flow uses its existing child workflow and typed-result boundary instead of adding an SDK-specific
  delegation ABI.

- [Agent Spec](https://oracle.github.io/agent-spec/26.1.2/agentspec/language_spec_26_1_2.html)
  defines a manager-workers pattern with explicit workers that report to the manager and do not
  interact with the user. It informs the specialist vocabulary but does not define Flow's durable
  evaluation or activation authority.
...
- This issue does not change model routes, tools, tool packages, approvals, recovery, policies,
  budgets, concurrency, graph topology, dependencies, result schemas, or isolation.
...
- This issue does not claim that one evaluated specialization generalizes beyond its declared tasks,
  seeds, fixtures, model, package closure, and verification controls.
...
- **Invalid input** — Invalid, ambiguous, no-op, multi-axis, stale, oversized, unstable, linked, or
  unrelated-changing input fails closed with a fixed stage and publishes no candidate or active
  state.

- **Missing context** — A missing active head, child target, agent target, baseline source, evidence
  packet, immutable package, selected skill, evaluation, policy, or runtime dependency stops the
  operation. Flow uses no live fallback.
...
- A child-specialist source has one versioned kind, metadata identity, workflow scope, embedded
  child node id, child agent node id, exact baseline identity, and one change union. The union
  contains either bounded instructions or a bounded, duplicate-free ordered Agent Skill selection.

- A child-specialist identity binds the manifest, root workflow, embedded child workflow, target
  agent, package-closure digest, declared axis, before and after values by content-free identity,
  projected complete workflow, and candidate digest.
...
- Public candidate, evaluation, activation, run, event, inspection, and export views contain only
  bounded target, axis, state, package, evidence, and digest identities. They contain no prompt or
  package content, absolute path, private response, secret, or nested cause.
...
- The issue and specification identify the target, the two exclusive axes, preserved authority,
  durability, public privacy, and excluded packaged-child and dynamic-agent work.
...
- The flow descriptions define review, evaluation, activation, runtime, recovery, cancellation, and
  settlement behavior.
- The coupling section identifies the owning layer for each responsibility and keeps child runtime
