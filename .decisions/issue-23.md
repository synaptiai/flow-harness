...
| Invalid config fails before mutation | Error/holdout | `npx vitest run test/integration/config/project-config.test.ts test/integration/cli/main.test.ts -t "configuration"` | Version, kind, unknown field, bound, widening, and YAML diagnostics include path; no supervisor/job/command state appears | Automatic config repair |
...
| Deterministic starvation-free restart order | State/model/runtime | `npx vitest run test/unit/supervisor/admission-reducer.test.ts test/integration/supervisor/service.test.ts -t "FIFO|restart"` | Every reachable state preserves unique FIFO dispatch; restart preserves sequence | Priority fairness |
| Bounded queue and durable rejection | Boundary/idempotency | `npx vitest run test/integration/supervisor/service.test.ts -t "bounds active|oversubscribes"` | Exactly Q waiters; overflow retry returns the same rejection and retains no job snapshot | Client-side retry policy |
...
| Bounded status and policy digest | API/security | `npx vitest run test/integration/supervisor/service.test.ts -t "status"` | Active/queued summaries and digest are bounded; no source/token/reason/provider load | Full historical queue export |
| Policy mismatch requires safe restart | Lifecycle | `npx vitest run test/integration/supervisor/daemon.test.ts test/runtime/cli-process.runtime.test.ts -t "policy mismatch"` | Changed effective values fail before submit; shutdown refuses non-idle state; explicit idle restart rebinds | Hot reload |
...
| Public documentation accuracy | Documentation | `npx vitest run test/scaffold/community-files.test.ts` | README and architecture/config/recovery/security/testing/roadmap describe implemented and deferred behavior | Future capabilities |
