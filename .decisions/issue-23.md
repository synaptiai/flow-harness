...
- `npm run check`: passed on the final tree; 41 default test files / 472 tests, clean build, and 2
...
- `npm run test:coverage`: passed the configured 75% statement, 65% branch, 70% function, and 75%
  line thresholds. Process-race coverage is timing-sensitive; the final recorded run reported 82.27%
  statements, 72.71% branches, 90.75% functions, and 82.61% lines.
- Package metadata inspection: 177 files, 219,433 bytes compressed, and 1,169,149 bytes unpacked.
...
| Final test, coverage, and package counts described an earlier commit as the final tree | P2 | Fixed: evidence was regenerated after the last code change and records 41/472 default tests, 2/14 runtime tests, enforced coverage thresholds with the latest observation, and current 177-file package sizes |
