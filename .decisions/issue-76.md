...
The driver awaits the pinned SDK session `disposeAsync()` operation before it sends the terminal
frame. The pinned in-memory authentication storage has no separate cleanup operation. The
supervisor kills and reaps every user 10002 process, including detached process groups.
