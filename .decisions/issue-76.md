...
custom IPython tool. The driver awaits the pinned SDK session `disposeAsync()` operation and then
awaits the caller-owned provisioner cleanup before it sends the terminal frame. The pinned
in-memory authentication storage has no separate cleanup operation. The supervisor kills and reaps
every user 10002 process, including detached process groups.
...
| Run a real persistent IPython session | Integration | `npx vitest run test/integration/prime/native-prime-agent-evaluation.test.ts` | One session keeps state across two turns. It uses one accepted kernel request. The caller-owned kernel settles before the driver exits. | Live provider quality |
