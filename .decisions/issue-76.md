...
| Reconcile effective controls | Security | `npm run build && npm run test:runtime -- test/runtime/prime-agent-oci-startup.runtime.test.ts && npx vitest run test/unit/infrastructure/oci/attached-prime-oci-operator.test.ts` | Each readiness group matches policy. Changed readiness rejects before fixture and secret transfer. Non-Error operation and cleanup rejections become fixed stage errors. | Host-kernel compromise |
