...
The Python closure contains a copied pinned base runtime and a virtual environment under one
`/opt/flow/python` root. The final Node image executes the virtual-environment interpreter and
imports the complete admitted Python package set before it creates the runtime users. This probe
rejects a closure that still depends on the Python build image. The image probe hashes the base
runtime and virtual environment together and reads the version from the nested virtual environment.
