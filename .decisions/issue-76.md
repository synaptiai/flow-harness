...
uses the caller-owned provisioner `kill()` operation before it sends the terminal frame. Version
one does not retain a kernel snapshot across trials. The pinned in-memory authentication storage
has no separate cleanup operation.
