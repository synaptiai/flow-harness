...
missing runtime diagnostic, and residual task creation. Missing-object categories distinguish bundle
write, bundle read, shim-log opening, shim-address publication, shim-identity recording, other shim
startup opens, OCI-create opens, and task-object opens. They also distinguish object creation,
residual object open, object inspection, link resolution, and working-directory resolution. Pinned
runtime-init categories distinguish init-binary preparation, init-process launch, isolated-device
preparation, init policy, and network defaults. During runc child initialization, fixed source
wrappers take priority over the opened object. A remaining missing open distinguishes descriptor,
AppArmor, device, kernel, process, and residual init-object families without publishing the path.
...
The Linux x64 acceptance runner selects `/usr/bin/runc` from the exact `containerd.io` 1.7.27-1
package. It verifies `runc` 1.2.5 commit `v1.2.5-0-g59923ef` and does not resolve the runtime through
`PATH`. This fixed selection keeps version one's complete `/proc/sys` mask. A different runtime
path, version, or commit is outside the verified acceptance profile.
