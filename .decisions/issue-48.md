...
| Progressive disclosure and authority | Behavioral/security | `npm test -- --run test/unit/infrastructure/pi/workspace-agent-tools.test.ts test/unit/infrastructure/pi/pi-agent-executor.test.ts` | Prompt contains metadata only; selected resources load on demand; requests never add tools/policy actions | Protection from malicious instruction meaning |
...
| Attached/detached/child/recovery parity | Behavioral | `npm test -- --run test/integration/cli/capability-execution.test.ts test/unit/supervisor/capability-jobs.test.ts` | Source drift after each admission boundary cannot change execution; children and resume reuse the same digest | Distributed package registries |
