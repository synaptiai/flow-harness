...
- DeepSeek Harness uses an append-only typed session-event log as its source of truth. Full request
  snapshots reconstruct exact replay surfaces.
...
- W3C PROV models derivation as a new entity related to prior entities and activities. Verified
  facts therefore retain evidence references. They do not copy evidence contents.
...
- **Timeouts and cancellation** — Local reads and evidence resolution check the caller signal at
  each asynchronous boundary. Cancellation before durable append changes no state. No background
  retry starts.
...
- **Invalid input** — Strict YAML or JSON parsing rejects aliases, duplicate keys, invalid Unicode,
  unknown fields, and invalid identifiers. It also rejects duplicate entries, invalid evidence
  references, and every limit excess before mutation.
...
- **Resource exhaustion** — Source bytes, entry counts, evidence references, and revision bytes have
  fixed limits. Revision count, ledger bytes, page size, JSON depth, and JSON nodes also have limits.

- **Corruption** — A changed digest, broken predecessor link, or noncontiguous revision rejects the
  complete read. Empty lines, invalid schemas, unsafe files, and exceeded bounds also reject it.
...
- **Settlement uncertainty** — Flow reopens the ledger after an append failure. It returns success
  ...
  A writer-lease release failure cannot produce clean success.

### Interface contracts

- A source document uses API version `flow.synapti.ai/v1alpha1` and kind `GoalWorkspace`. It has one
  objective, bounded entry groups, and exactly one `nextAction`.
...
- A committed revision has version `1`, a positive contiguous revision, an optional predecessor
  digest, and a UTC timestamp. It contains the complete state and its canonical SHA-256 digest.
...
- Agent context contains the objective and text entries but no evidence locator contents,
  supplemental-memory contents, or run evidence. It identifies the workspace as reference context.
  The context cannot override the node prompt or Flow authority.

- Public projections can show revision identity and evidence locators. They never show raw evidence
  or supplemental-memory contents.
...
6. Update public documentation and architecture. Run mapped and complete gates. Merge only at zero
...
- The exact 13-file portable selector passed 175 tests. It covers the domain, application, source,
  store, and capability snapshot. It also covers run propagation, public output, prompts, CLI, and
...
- `npm run pack:check` installed the generated package and ran its CLI. The package archive SHA-256
  digest was
...
- The runtime selector passed 37 tests in seven files. It skipped 37 tests in 11 files. Seven tests
  missed their local deadlines on this host.

- The native Pi runtime file passed three tests in isolation, with two skipped. The full CLI process
  file passed 13 of 16 tests in isolation. Its remaining browser, capacity, and rebinding cases
  retained existing process deadlines. No goal-workspace runtime path failed.

These results establish the Issue #151 behavior and package boundary. The local runtime exceptions
remain explicit. Hosted CI remains the independent runtime gate.

## Adversarial review

Review covered specification compliance, execution authority, and public-data projection. It also
covered evidence identity, replay, filesystem identity, CAS behavior, cancellation, resource bounds,
settlement, documentation claims, and test mutation resistance.
