...
3. Each relationship addition uses one closed predicate, two exact entry-version endpoints in the
   same target scope, and from one to four durable run-event evidence locators.
4. Flow reopens the candidate without following links and resolves every evidence locator to one
...
- The [MCP reference memory server](https://github.com/modelcontextprotocol/servers/tree/main/src/memory)
  demonstrates a simple entity-relation graph, but arbitrary relationship strings and mutable live
  graph operations do not satisfy Flow's evidence, versioning, evaluation, or rollback contracts.
...
- [J-Space Cognition Suite](https://github.com/Tiger3807861189/J-Space-Cognition-Suite-V3.6)
  motivates keeping contradictions visible rather than silently choosing a winner.
...
Let `R` be relationships in one state, `D_e` the incident degree of one active entry, `C` the
relationship changes in one candidate, `V_r` evidence references on one relationship, `V_s` total
evidence references in one state, `B_r` serialized relationship metadata, and `B_p` the rendered
model-visible relationship summary.
...
The evidence bound follows directly from `32 × 4 = 128`. The degree bound means one candidate can
remove four prior incident relationships and add four rebound relationships, so the eight-change
candidate bound is sufficient for a complete replacement. The relationship metadata bound is
twice the 65,536-byte memory-content bound but remains less than 0.8% of the 16 MiB complete-state
limit. All larger state, runtime, supervisor-frame, and provider-context limits remain independent.
...
| Lineage creates a cycle | Reject `refines` or `derived_from` cycles; do not infer a hierarchy. |
...
- Relationship data is optional. Absence preserves every historical identity and prompt byte.
...
- **Expected evidence**: exact bounds pass and plus-one values fail; every invalid graph form rejects
  before a projection is returned.
...
- **Command**: `npx vitest run test/integration/cli/supplemental-memory-relationships.test.ts test/integration/cli/effective-harness-composition.test.ts test/integration/cli/effective-harness-runtime.test.ts`
...
- **Does not promise**: live graph repair or provider conversation persistence.

### Criterion 7 — Bounded targeted prompt projection

- **Type**: runtime and privacy behavior
- **Command**: `npx vitest run test/unit/adaptation/supplemental-memory.test.ts test/unit/infrastructure/pi/pi-agent-executor.test.ts test/integration/cli/effective-harness-runtime.test.ts`
- **Expected evidence**: only the exact target receives canonical bounded metadata, with no evidence
  locators or unrelated memory.
- **Does not promise**: that a model will obey or correctly reason about the relationships.

### Criterion 8 — Public privacy

- **Type**: presentation and adversarial behavior
- **Command**: `npx vitest run test/unit/cli/public-output.test.ts test/integration/cli/supplemental-memory-relationships.test.ts`
- **Expected evidence**: public output excludes raw and encoded canaries, locators, paths, and nested
  private causes while retaining counts and digests.
...
- **Expected evidence**: detailed guidance is routed outside README, links resolve, prose gates pass,
  and architecture and roadmap contracts are current.
- **Does not promise**: support for deferred live retrieval or graph protocols.
...
- **Commands**:
  ...
  - Hosted Linux x64 CI
- **Expected evidence**: every portable gate passes, coverage stays above repository thresholds,
  packaged files are intentional, production dependencies have no known alert, and hosted CI is
  green.
- **Does not promise**: a live-provider test when local credentials are unavailable.
