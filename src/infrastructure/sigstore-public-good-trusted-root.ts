...
export const SIGSTORE_PUBLIC_GOOD_TRUSTED_ROOT_SOURCE = Object.freeze({
...
const SIGSTORE_PUBLIC_GOOD_TRUSTED_ROOT_JSON = {
...
export function createSigstorePublicGoodTrustedRoot(): TrustedRoot {
  const root = TrustedRoot.fromJSON(SIGSTORE_PUBLIC_GOOD_TRUSTED_ROOT_JSON);
  const canonical = Buffer.from(JSON.stringify(TrustedRoot.toJSON(root)));
  if (
    canonical.byteLength !== SIGSTORE_PUBLIC_GOOD_TRUSTED_ROOT_SOURCE.canonicalBytes ||
    createHash("sha256").update(canonical).digest("hex") !==
      SIGSTORE_PUBLIC_GOOD_TRUSTED_ROOT_SOURCE.canonicalSha256
  ) {
    throw new Error("shipped Sigstore trusted root identity mismatch");
  }
  return root;
}
