...
describe.skipIf(!enabled)("host bridge active-descendant settlement", () => {
  ...
  it("settles the live relay leader and connection child before test socket cleanup", async (context) => {
    ...
    try {
      ...
      child.stdout.on("data", (bytes: Buffer) => {
        ...
        if (output !== completeRecords || receiptSeen) return;
        receiptSeen = true;
      ...
      await requiredWithin(forwarded.promise, 1000, "Actual bridge forwarding did not complete");
...
function settled(record: BridgeCheck): boolean {
